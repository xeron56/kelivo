    ...
    try {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        for (final e in data) {
          ...
          // Filter out audio and safety models
          if (lowerId.contains('whisper') ||
              lowerId.contains('guardian') ||
              lowerId.contains('guard'))
            continue;

          // Check 'active' flag if present (user API response has active: true)
...
class ProviderManager {
  ...
  static Future<void> testConnection(
    ...
    final client = _Http.clientFor(cfg);
    try {
      if (kind == ProviderKind.openai || kind == ProviderKind.groq) {
        final base = cfg.baseUrl.endsWith('/')
            ? cfg.baseUrl.substring(0, cfg.baseUrl.length - 1)
            : cfg.baseUrl;
