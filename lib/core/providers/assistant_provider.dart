...
class AssistantProvider extends ChangeNotifier {
  ...
  static const String financeAssistantId = 'builtin_finance_assistant';
  static const String financeAssistantName = 'Finance Assistant';
  ...
  Assistant? get financeAssistant {
    final idx = _assistants.indexWhere((a) => a.id == financeAssistantId);
    if (idx == -1) return null;
    return _assistants[idx];
  }

  bool isFinanceAssistantId(String? id) {
    if (id == null) return false;
    if (id == financeAssistantId) return true;
    final idx = _assistants.indexWhere((a) => a.id == id);
    if (idx == -1) return false;
    return _assistants[idx].name.trim().toLowerCase() ==
        financeAssistantName.toLowerCase();
  }
  ...
  Assistant _financeAssistant() => const Assistant(
    id: financeAssistantId,
    name: financeAssistantName,
    systemPrompt:
  ...
  Future<void> ensureDefaults(dynamic context) async {
    ...
    bool changed = false;
    final Assistant financeTemplate = _financeAssistant();
    ...
    final int financeIdx = _assistants.indexWhere(
      (a) => a.id == financeAssistantId,
    );
    if (financeIdx == -1) {
      _assistants.add(financeTemplate);
      changed = true;
    } else {
      final Assistant existing = _assistants[financeIdx];
      if (existing.name != financeTemplate.name ||
          existing.systemPrompt != financeTemplate.systemPrompt ||
          existing.deletable != financeTemplate.deletable ||
          existing.temperature != financeTemplate.temperature ||
          existing.topP != financeTemplate.topP) {
        _assistants[financeIdx] = existing.copyWith(
          name: financeTemplate.name,
          systemPrompt: financeTemplate.systemPrompt,
          deletable: financeTemplate.deletable,
          temperature: financeTemplate.temperature,
          topP: financeTemplate.topP,
        );
        changed = true;
      }
    }
